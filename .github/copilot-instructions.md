...
## Project Overview
- **Purpose:** Website for sharing birthday information, including non-public data (e.g., live location) behind authentication.
- **Hosting:** Cloudflare Worker (serverless), with Cloudflare KV for storing non-public data.
- **Authentication:** Basic Auth is used to restrict access.

## Developer Workflows
TBD

## Patterns & Conventions
- **KV Binding:** Always use the binding name `BIRTHDAY_KV` in Worker code for accessing the namespace.
- **Minimal Dependencies:** The project is designed to be simple and cost-effective (Cloudflare free tier).

## Integration Points
- **Cloudflare:** All hosting, authentication, and data storage are via Cloudflare services.
- **No Static Hosting:** The site is not a static site; all requests are handled by the Worker.

## Example: Accessing KV in Worker
```js
// In src/worker.js
...
async function handleRequest(request) {
  const value = await BIRTHDAY_KV.get('some-key')
  // ...
}
```

---
If you are unsure about any workflow or convention, check the `README.md` for up-to-date project details.
